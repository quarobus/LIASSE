...
export default function ModalPopUpSujets() {
  ...
  function getUsers() {
    ...
    if (email) {
      axios.get(`http://localhost:8080/api/ProfThese.php?email=${email}`).then(function(response){
      ...
      {modal && (
        <div className="modal-PopUp">
          ...
          <div className="modal-content-PopUp">
            <br></br>
          <p><strong>Les sujects de thèse</strong></p>
            {Array.isArray(users) ? (
                users.map((user,key) => 
                <div key={key}>
                      <p>{user.title}</p>
