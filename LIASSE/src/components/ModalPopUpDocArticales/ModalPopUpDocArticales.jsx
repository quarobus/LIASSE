...
export default function ModalPopUpDocArticales() {
  ...
  function getUsers() {
    ...
    if (email) {
      axios.get(`http://localhost:8080/api/DocArticales.php?email=${email}`).then(function(response){
      ...
      {modal && (
        <div className="modal-PopUp">
          ...
          <div className="modal-content-PopUp">
            <br></br>
          <p><strong>Les articles réalisés</strong></p>
          {Array.isArray(users) ? (
              users.map((user,key) => 
              <div key={key}>
                    <p>{user.title}</p>
